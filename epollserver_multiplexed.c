...
struct environment {
    struct ringbuffer **wires; 
    int wire_count;
    hashmap *hashmap;
};

struct environment_entry {
    struct environment_subentry **subentries;
    int len; 
};

struct environment_subentry {
    char *entry;
    int len;
};
...
int environment_wire(struct environment_entry *environment_entry, struct environment *environment, char * entry) {
	struct ringbuffer *io_ringbuffer = calloc(1, sizeof(struct ringbuffer));
	   io_ringbuffer->size = PENDING_SIZE;
	   io_ringbuffer->last_head = -1;
	   io_ringbuffer->last_tail = -1;
	   io_ringbuffer->tail = 0;
	   io_ringbuffer->head = 0;
    environment->wires[environment->wire_count] = io_ringbuffer; 
    environment->wire_count++;
    hashmap_set(environment->hashmap, entry, strlen(entry), environment_entry);
}

int environment_init(struct environment *environment) {
    environment->hashmap = hashmap_create(); 
    environment->wire_count = 0;
	struct ringbuffer *wires = calloc(100, sizeof(struct ringbuffer *));
...
void free_entry(void* key, size_t ksize, uintptr_t value, void* usr)
{
	...
	printf("Entry \"%s\": %i\n", key, value);
    void *environment_entry = (void*) value;
    struct environment_entry *entry = (struct environment_entry *) environment_entry;
    for (int i = 0 ; i < entry->len; i++) {
        free(entry->subentries[i]->entry);
        free(entry->subentries[i]);
    }
    free(key);
...
int environment_free(struct environment *environment) {
    for (int i = 0 ; i < environment->wire_count; i++) {
        free(environment->wires[i]);
    }
    hashmap_iterate(environment->hashmap, free_entry, NULL);
    hashmap_free(environment->hashmap); 
    free(environment->wires);
    free(environment);
}
...
int main() {
    ...
    environment = calloc(1, sizeof(struct environment));
    ...
    for (int i = 0; i < thread_count; i++) {
        int size = 0;
        struct environment_entry *environment_entry = calloc(1, sizeof(struct environment_entry));
        struct environment_subentry *environment_subentry = calloc(1, sizeof(struct environment_subentry));
        char *thread = calloc(25, sizeof(char));
        sprintf(thread, "thread%d", i);
        printf(thread);
        size++;
        environment_entry->subentries = calloc(size, sizeof(struct environment_subentry));
        environment_subentry->len = strlen(thread);
        environment_subentry->entry = thread;
        // strlcpy(environment_subentry->entry, thread, (size_t) environment_subentry->len);
        environment_entry->subentries[environment_entry->len] = environment_subentry;
        environment_entry->len++;

        environment_subentry = calloc(1, sizeof(struct environment_subentry));
        char *server = calloc(25, sizeof(char));
        sprintf(server, "server%d", i);
        printf(server);
        environment_subentry->len = strlen(server);
        size++;
        environment_subentry->entry = server;
        char *fullname = calloc(100, sizeof(char));
        snprintf(fullname, 100, "%s %s", thread, server);
        // strlcpy(environment_subentry->entry, server, (size_t) environment_subentry->len);
        environment_entry->subentries[environment_entry->len] = environment_subentry;
        environment_entry->len++;
        environment_wire(environment_entry, environment, fullname);
    }
